...
def _load_json_file(json_path, config):
    ...
    with open(json_path, "r",encoding="utf8", errors='ignore') as file:
        ...
        for line in file:
            try:
                ...
                labels.append([float(item)/4 for item in entry["sentiment"]])
                features.append([float(item) for item in entry["features"]])
            except:
                continue
